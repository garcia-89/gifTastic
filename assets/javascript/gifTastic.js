...
var topics = ["BMW", "Chevrolet", "Lamborghini", "Maserati", "Ford", "AMG", "Lexus", "Aston Martin", "Nissan", "Honda", "Toyota", "Jeep","Bentley"];
...
var buttonGenerator = function () {
	...
	for (i = 0; i < topics.length; i++) {
		button = $("<button type=" + "button" + ">" + topics[i] + "</button>").addClass("btn btn-secondary").attr("data", topics[i]);
		$("#buttonArea").append(button);
	};
}


// The user clicks on a generated gray button, which generates 10 static, non-animated gif images from the GIPHY API and places them on the page. 
$("#buttonArea").on("click", ".btn", function () {
	...
	$.ajax({
		...
		for (var i = 0; i < results.length; i++) {
			...
			var topicImage = $("<img>").addClass("gifBorders");
